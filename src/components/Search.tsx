...
function rankItem(item: SearchItem, query: string, getTitle: (key: string) => string, getDesc: (key: string) => string): number {
        ...
        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-[#e8e6e1] rounded-xl shadow-lg shadow-black/5 overflow-hidden z-50">
          {results.map((item, i) => (
            ...
            >
              ...
              <span className="text-sm text-[#1a1a18]">{getTitle(item.titleKey)}</span>
              <span className="text-xs text-[#a8a49d] line-clamp-1">{getDesc(item.descriptionKey)}</span>
            </Link>
          ))}
        </div>
      )}
