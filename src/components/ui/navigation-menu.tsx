...
function NavigationMenu({
  ...
  return (
    <NavigationMenuPrimitive.Root
      ...
      className={cn(
        "max-w-max group/navigation-menu relative flex flex-1 items-center justify-center list-none",
        className,
      )}
      {...props}
    >
...
function NavigationMenuViewport({
  ...
  return (
    <div
      className={cn(
        // Ставим фиксированное позиционирование и left-0, чтобы привязаться к краю экрана
        "fixed top-18 left-0 isolate z-50 flex w-full justify-center",
      )}
    >
      <NavigationMenuPrimitive.Viewport
        data-slot="navigation-menu-viewport"
        className={cn(
          "bg-popover text-popover-foreground ring-foreground/10   duration-100 origin-top-center relative overflow-hidden", //data-open:animate-in data-closed:animate-out data-closed:zoom-out-95 data-open:zoom-in-90
          "h-[var(--radix-navigation-menu-viewport-height)] w-full ", // Убираем скругления и md-ширину здесь
          // Фон просто плавно проявляется (fade), без движений
          "data-open:animate-in data-open:fade-in data-closed:animate-out data-closed:fade-out duration-200 shadow-sm ",
          className,
        )}
        {...props}
      />
